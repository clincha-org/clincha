...
Bucket `clincha-hawkstore-vault`, credential `Backblaze B2 - hawkstore vault` (id 2).
...
A killed run leaves an orphan `<dataset>@cloud_sync-<taskid>-<timestamp>` snapshot behind.
Destroy it; the next run takes a fresh one.

### Why this is not a systemd timer

The first iteration was a `systemd` timer running a hand-rolled `rclone` script. It was replaced
on 2026-08-07 because a script has no failure alerting (it could fail nightly and silently for
months), no job history, and lived in `/root` and `/etc/systemd/system` on the boot environment.

That last point stopped being theoretical the same afternoon. TrueNAS auto-updated 25.10.4 →
25.10.5 and booted into a fresh boot environment: **`/etc/systemd/system/offsite-backup.*` was
gone**, while the cloud sync tasks came through untouched because they live in the config
database. Had the migration not already happened, the off-site backup would have quietly ceased
to exist, with nothing to announce it.

The same reboot killed an upload in flight. The task simply resumed and re-uploaded only the
remainder — which is the other half of the argument for a mechanism that has state and
supervision rather than a one-shot script.
...
**The two crypt values exist only in Bitwarden**, item
`Backblaze - hawkstore offsite backup v2` (`crypt password` and `crypt password2`). This is the
whole point: a key stored on hawkstore would be worthless in exactly the scenario the off-box
copy is for. If that vault item is lost, the bucket is unrecoverable ciphertext — there is no
recovery path and no support ticket that fixes it.

The `v2` is not decoration. The original bucket, application key and crypt pair were **retired
on 2026-08-07 after the crypt password and salt were exposed in cleartext** — an agent piped a
file of plaintext secrets into `ssh … bash -s` badly enough that the shell tried to execute them
and echoed both in its error output. Rotation was a fresh bucket, a fresh scoped key, a fresh
crypt pair generated by hand, a full re-upload, and destruction of the old bucket. Two lessons
worth keeping:

- Move secrets as **files** (`cat > file` with a stdin redirect), never interpolated into a
  command string, and confirm them by length rather than value. Any command that fails may echo
  its arguments.
- **`cloudsync.create` and `cloudsync.credentials.query` return the B2 `key` in full.** Printing
  an ordinary success response leaks the credential. Filter before logging.
...
rclone config create immich crypt remote b2raw:clincha-hawkstore-vault/immich \
...
password and salt, so only the remote path changes. Note the B2 `key` goes in **raw** while the
crypt values must be **obscured** — see the trap below.

Exercised end-to-end on 2026-08-07 from a machine that is not hawkstore, rebuilding the config
from the vault alone and restoring a 2.5 MB file whose SHA256 matched the original. Re-test
whenever the credential changes — an untested off-box backup is a belief, not a backup.

### What has actually been verified

Not everything got the same level of proof, and the difference matters:

| dataset | verification |
|---|---|
| `data/family-archive` | **`rclone cryptcheck`, 28,043 files, 0 differences.** Full hash comparison. |
| `data/immich` | object count + byte total reconciled against source |
| `userstore/userhome` | object count + byte total reconciled against source |
| `userstore/s3` | object count + byte total reconciled against source |

`family-archive` gets the strong check because it is the one dataset with **no other copy** —
its B2 source buckets were deleted. The other three still have local ZFS replication to
`backups`, and they are live datasets, so hashing them against a snapshot-time upload would
produce differences that are drift rather than corruption. Their integrity rests on rclone
verifying each object's SHA1 against B2 at upload time.

Use `rclone cryptcheck`, not `rclone check`: **`check` silently degrades to size-only against a
crypt remote**, because crypt cannot expose a plaintext hash. `cryptcheck` computes the expected
encrypted hash per file and is the only one that proves content.
