...
![packer](https://github.com/clincha/clinch-home/actions/workflows/packer.yml/badge.svg)
![terraform](https://github.com/clincha/clinch-home/actions/workflows/terraform.yml/badge.svg)
...
The feeling of paradise slowly wore down as the process of booting up new VMs and configuring each every time became tiring. I was in a much better position than before, but the twenty minutes it now took to get a new machine was feeling a lot like the two hours had before. I needed to cut that down.
...
The first tool I wanted on there were GitHub runners. I had manually created a few before the first time I spoke the words infrastructure as code, and they were showing their age. I had used the server as a test server a few times and some strange inconsistencies with Python had lost me hours one evening. I found a really useful Ansible role to install the runners and configured it with my parameters, and before I knew it they were running and ready.

![runners.png](Documentation/images/README/runners.png)

Next on the list was Kubernetes. It would be the final part of the infrastructure stack to install and mark the end of the main features needed for this repository. Eventually, all applications that I create should sit in these clusters. Writing about it now I feel a little pang of sadness, which is weird to feel for a bunch of code and some metal. It's been a process though and one that not many undertake. There are so many downsides: the cost, the time commitment, the environmental impact the inflexibility of the solution. However, it's been a journey and one that I've leaned so much from. I've had fun installing a rack with Roger, negotiating with my parent on noise levels and getting to nerd out with my girlfriend who listens so diligently. It might not be the best way to get a Kubernetes cluster up and running but looking back, each moment along the way was worthy.

I created an Ansible role for the cluster to round out the project. I couldn't have dreamed of creating this role when I started out. I had tried before and failed after installing the completely wrong dependencies for my operating system. Something I needed time away to figure out. This time though, it was simple. I knew what I needed to do through research and I knew where to go for help. I even managed to contribute to another repository when I found a minor bug. Ansible ran, Kubernetes installed, and the cluster was ready.  